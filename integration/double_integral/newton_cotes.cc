...
double NewtonCotes::estimate(double x_I, double x_F, double precision){
    int n = 1;
    ...
    do{
        I_old = I;
        n = 2*n;
        I = calculatePartitions(x_I, x_F, n);
    } while(std::abs(I - I_old) > precision);

    return I;
}

double NewtonCotes::calculatePartitions(double x_I, double x_F, int number_of_partitions){
    double I = 0;
